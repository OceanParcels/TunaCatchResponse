...
if(__name__ == '__main__'):
    ...
    ff = 'BJ'  # flow field configuration
    ...
    gridx, gridy = np.meshgrid(np.arange(-res, lx+res, res),
                               np.arange(-res, ly+res, res))
    gridx = np.array(gridx) + 0.5 * res
    gridy = np.array(gridy) + 0.5 * res
    fieldset = FieldSet.from_data({'U': np.zeros(dataP.shape),
                                   'V': np.zeros(dataP.shape)},
                                  {'lon': gridx,
                                   'lat': gridy},
                                   mesh = 'flat')
    ...
    if(nfad > 0):
        ...
        fieldF = Field('FADorders', np.arange(nfad), lon=np.arange(nfad),
                       lat=np.array([0]), time=np.array([0]),
                       interp_method='nearest', mesh='flat',
        ...
        fieldF = Field('FADc', np.array([0]), lon=np.array([0]),
                       lat=np.array([0]), time=np.array([0]),
                       interp_method='nearest', mesh='flat',
                       allow_time_extrapolation=True)
        fieldset.add_field(fieldF)  # prey field added to the velocity FieldSet
        fieldset.FADc.to_write = False  # enabling the writing of Field prey during execution
    fieldFe = Field('fe', np.array([0]), lon=np.array([0]), lat=np.array([0]),
                    time=np.array([0]),
                       interp_method='nearest', mesh='flat',
                       allow_time_extrapolation=True)
    ...
    print('realistic FAD-tuna interaction distance is around 10km (7Nm), now (km):',
    ...
    output_file = pset.ParticleFile(name="output%s/FADPrey%s_no%d_npart%d_nfad%d_T%.2f_F%.2f_P%.2f_I%.2f_p%.1f_Pa%.1f.nc"%(ff,
                                                                                                                           ff,
                                                                                                                           seed,
                                                                                                                           npart,
                                                                                                                           nfad,
    ...
    if(ff == 'BJ'):
        ...
        kernels += pset.Kernel(pk.PreyGrad_zpb)
