...
__all__ = ['Iattraction', 'ItunaFAD', 'ItunaFAD_zpb', 'Itunatuna',
           'Itunatuna_zpb',
           'ItunaPredFAD', 'ItunaPredFAD_mC', 'Stcheck',  # particle-particle interaction kernals
           'PreyDeplete', 'GEvacuation',  # particle-field interaction kernel
           'DoubleGyre', 'BickleyJet', 'DiffusionUniformKhP',  # flow kernels
...
def Iattraction(particle, fieldset, time, neighbors, mutator):
    ...
    if(particle.id == 0 and fieldset.p >= 0):
...
def ItunaFAD(particle, fieldset, time, neighbors, mutator):
    ...
    if(fieldset.kappaF != 0 and particle.ptype == 0 and fieldset.nfad > 0): # if tuna swims towards FAD
        ...
        if(DS!=[0, 0]):
            ...
            d_vec = VP
            def f(particle, dlat, dlon, ddepth):  # define mutation function for mutator
                particle.dla += dlat
                particle.dlo += dlon
            mutator[particle.id].append((f, d_vec))  # add mutation to the mutator

    return StateCode.Success


def Stcheck(particle, fieldset, time, neighbors, mutator):
    # updates the integrated stomach emptiness in the cases that associated with FAD or not
    if(particle.ptype == 0):
        ...
        for n in neighbors:
            if(n.ptype == 1 and n.id != 0 and bo):
                ...
                if(l_n > (particle.lon-(n.lon+fieldset.Lx))**2):
                    x_p = np.array([n.lat, n.lon + fieldset.Lx, n.depth]) # n location
...
def ItunaPredFAD(particle, fieldset, time, neighbors, mutator):
    ...
    if(day%particle.dt != 0):
        print('no fishing taking place!!!!!')

    # Set the fishing location if p==-1
    if(np.isclose(time % day, day-particle.dt) and fieldset.p == -1):
        if(particle.id == 0):
            fieldset.fe.data[0, 0, 0] = random.randint(fieldset.nfad+1,
                                                       fieldset.nfad+1+fieldset.ntuna)
    ...
    if(particle.ptype == 1 and time > 0 and np.isclose(time % day, 0)):  # if FAD, consume tuna with a probability
        if(particle.id == 0 and fieldset.p == -1):
            ...
            prob = 0
        elif(fieldset.nfad == 1 and particle.id == 1):
            prob = 1
        else:
...
def ItunaPredFAD_mC(particle, fieldset, time, neighbors, mutator):
    """The predation of tuna near a FAD"""
    # The geometric function
    def geometric(x, p=fieldset.p):
        return (1-p)**(x-1)*p
        ...
        if(prob > 0):
            ...
            for n in neighbors:
                if(n.ptype == 0):
                    ...
                    x_n = np.array([n.lat, n.lon, n.depth]) # tuna location
                    dist = np.linalg.norm(x_p-x_n)
                    if(dist < fieldset.RtF and (random.random() < prob and totC <= fieldset.maxC)):
                        totC += 1
                        mutator[particle.id].append((fcF, [1]))  # FAD catches tuna particle
                        mutator[n.id].append((fcN, [1]))  # tuna particle is caught
    ...
    if(fieldset.kappaT != 0 and particle.ptype == 0): # if tuna swims towards FAD
        ...
        for n in neighbors:
            if n.ptype == 0:
                l_n = (particle.lon-n.lon)**2
                if(l_n > (particle.lon-(n.lon+fieldset.Lx))**2):
                    x_n = np.array([particle.lat, particle.lon + fieldset.Lx, particle.depth]) # n location
                    l_n = (particle.lon-(n.lon+fieldset.Lx))**2
                if(l_n > (particle.lon-(n.lon-fieldset.Lx))**2):
                    x_n = np.array([particle.lat, particle.lon - fieldset.Lx, particle.depth]) # n location
                    l_n = (particle.lon-(n.lon-fieldset.Lx))**2
            ...
            for n in range(3):
                if(bo):
                    ...
                    if(fieldset.flowtype == 'RW'):
                        ...
                        i = 1+np.random.binomial(fieldset.prey.data[0].shape[1]-2, 0.3)
                        j = 1+np.random.binomial(fieldset.prey.data[0].shape[0]-2, 0.5)
                    else:
                        i = np.random.randint(1, fieldset.prey.data[0].shape[1]-1)
                        j = 1+np.random.binomial(fieldset.prey.data[0].shape[0]-2, 0.5)
...
def CaughtP(particle, fieldset, time):
    # Reposition the tuna or prey to a random location, if it has been caught
    if(particle.ptype != 1):
...
def PreyGrad_zpb(particle, fieldset, time):
    if(particle.ptype == 0):
        ...
        particle.gradx = (fieldset.prey[0, 0, particle.lat,
                                        particle.lon+gradresr] -
                fieldset.prey[0, 0, particle.lat,
                              particle.lon-gradresl]) / fieldset.gres
        particle.grady = (fieldset.prey[0, 0, particle.lat+gradresu,
                                        particle.lon] -
                fieldset.prey[0, 0, particle.lat-gradresd,
                              particle.lon]) / fieldset.gres
...
def FaugerasDiffusion(particle, fieldset, time):
    if(particle.ptype == 0):
        ...
        kappaM = fieldset.alpha * np.linalg.norm([particle.gradx*fieldset.gres,
                                                  particle.grady*fieldset.gres])  # standard deviation angle
